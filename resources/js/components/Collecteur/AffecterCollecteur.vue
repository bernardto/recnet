        ...
        <v-row>
            <v-col cols="6">
                    ...
                    ><span class="headline" style="color: white">{{titre_data_table}}</span>
                        ...
                        <v-menu offset-y transition="scale-transition">
                            ...
                            <v-list>
                                ...
                                <v-list-item class="hover_blue" v-if="aff_reaf_des !== 2" @click="onclikGererAffDesRea(2)">
                ...
                <div   v-if="aff_reaf_des > 1 && data_user.length <= 0">
                    ...
                    <p class="headline mt-6">Liste vide! Choisissez les collecteurs à {{aff_reaf_des === 2? 'réaffecter': 'désafecter'}}.</p>
            ...
            <v-col cols="6">
                ...
                <v-card>
                    ...
                    ><span class="headline"> Zones & Quartiers </span></v-toolbar>
                ...
                >
                    ...
                    <template v-if="aff_reaf_des !== 1" v-slot:top>
                        <v-toolbar
                            flat
                        >
                            <v-toolbar-title>Liste des collecteurs {{ value_quartier.length > 1  ? ' des quartiers' : 'du quartier' }} sélectionné{{ value_quartier.length > 1  ? 's' : '' }}. </v-toolbar-title>
                    ...
                    <template v-slot:no-data>
                        <p class="subtitle-1">Ce quartier n'a aucun collecteur</p>
                    </template>
...
<script>
export default {
    name: "AffecterCollecteur",

    mounted() {


        this.$root.$emit('TITRE', "AFFECTATIONS DES COLLECTEURS NON AFFECTÉS ");
    ...
    methods: {
        ...
        onclikGererAffDesRea(val){
            ...
            switch (val){
                    case 1:
                        ...
                        this.getInfoUser();
                        this.titre_data_table = "Liste des collecteurs non affectés";
                    break;
                case 2:
        ...
        validerReAffection(add_ecra){
            ...
            axios.post('/valider_reaffection_collecteur', {
        ...
        validerAffection() {
            ...
            this.load_send = true;
            axios.post('/valider_affection_collecteur', {
                id_coll: this.data_affectation.map(a => a.id),
                id_qt: this.value_quartier.map(a => a.id)
            })
                .then(response => {
        ...
        getListeDesCollecteursDuQuartier() {
            ...
            axios.post('/get_collecteur_du_quartier', {id_qt: this.value_quartier.map(a => a.id)}).then(response => {
        ...
        getQuartierDesZones() {
            ...
            axios.post('/get_quartier_des_zones', {id_zn: this.value_zone.map(a => a.id_zn)})
                .then(response => {
                ...
                switch (this.aff_reaf_des){
                    case 2:
                      ...
                      this.data_r_affectation.forEach((item_aff, index) => {
                        ...
                        item.nv_qt = this.value_quartier.map(a => a.id);
