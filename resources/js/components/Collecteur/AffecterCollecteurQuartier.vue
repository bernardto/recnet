<template>
    ...
    <v-card fluid :disabled="load_send || loading_data || load_data_quartier">
        <v-row fluid>
        ...
        <v-row>
            <v-col cols="6">
                ...
                <v-card>
                    ...
                    ><span class="headline" style="color: white">{{titre_data_table}}</span>
                        ...
                        <v-menu offset-y transition="scale-transition">
                            ...
                            <v-list>
                                ...
                                <v-list-item class="hover_blue" v-if="aff_reaf_des !== 2" @click=" data_collecteur = []; onclikGererAffDesRea(2)">
                    ...
                    <v-data-table
                        :headers="headers"
                        v-if="!loading_data && this.data_collecteur.length > 0"
                    ...
                    >
                        ...
                        <template v-slot:no-data>
                            <p class="subtitle-1">Aucun collecteur dans la base de donnés</p>
                        </template>
                ...
                <div   v-if="aff_reaf_des > 1 && data_collecteur.length <= 0">


                    <p class="headline mt-6">Liste vide! Choisissez les collecteurs à {{aff_reaf_des === 2? 'réaffecter': 'désaffecter'}}.</p>
            ...
            <v-col cols="6">
                ...
                <div   v-if="aff_reaf_des === 1 && data_affectation.length <= 0">
                    ...
                    <p class="text-h6 mt-6">Liste vide! Choisissez les collecteurs à affecter au(x) quartier(s) sélectionné(s)</p>
                ...
                <v-data-table
                    :headers="headers2"
                    v-show="data_affectation.length > 0 && !load_data_quartier"
                    :items="data_affectation"
                    @click:row="clickRowAff"
                    :aria-disabled="load_send || load_data_quartier"
                    sort-by="id"
                    hide-default-footer
                    class="elevation-1"
                >
                    ...
                    <template v-if="aff_reaf_des !== 1" v-slot:top>
                        <v-toolbar
                            flat
                        >
                            <v-toolbar-title class="font-weight-black">Liste des collecteurs {{ value_quartier.length > 1  ? ' affectés aux  quartiers' : 'affecté à la quartier' }} sélectionné{{ value_quartier.length > 1  ? 's' : '' }}. </v-toolbar-title>
                ...
                <div v-show="value_quartier.length > 0 && aff_reaf_des > 1 && data_affectation.length <= 0">
                    <p  class="headline font-weight-black">Aucun collecteur est affecté à {{value_quartier.length > 1 ? "ces ("+ value_quartier.length+"  quartier )" : "ce quartier "}}</p>
            ...
            <v-btn
                ...
                fixed
                bottom
                v-if="data_affectation.length >  0 && aff_reaf_des === 1 && value_quartier.length > 0"
                right
                :loading="load_send"
            ...
            <v-btn
                ...
                fixed
                bottom
                v-if="data_r_affectation.length > 0 && aff_reaf_des === 2  && value_quartier.length > 0"
                right
                :loading="load_send"
        ...
        <v-row justify="center">
            ...
            >
                <v-card>
                    <v-card-title class="headline">
                        {{data_r_affectation.length> 1 ? 'Attention les collecteurs ne seront plus rattachés à une quartier ' :
            ...
            >
                <v-card>
                    <v-card-title class="text-h6">
                        {{ data_d_affectation.length > 1  ? 'Les collecteurs' : 'Le collecteur' }} sélectionné{{ data_d_affectation.length > 1  ? 's' : '' }}
                        {{ data_d_affectation.length > 1  ? 'ne seront' : 'ne sera' }} plus affecté{{ data_d_affectation.length > 1  ? 's' : '' }}
                        {{value_quartier.length <= 1?'à  la quartier selectionné' : 'aux quartiers selectionnés'}}.
                    </v-card-title>
                    <v-card-text>
...
export default {
    ...
    data: () => ({
        ...
        errorText: {quartier: '', '': false},
        message_snackbar: '',
        message_dialog_attention :"",
        afficher_snackbar:  false,
        errorBollean: {quartier: false, zone: false},
    }),
    ...
    methods: {
            ...
            axios.post('/valider_reaffection_collecteur_quartier', {
                ...
                .then(response => {
                    ...
                    if (response.data.success) {
                        ...
                        this.message_snackbar = "La réaffectation a été effectué."
                        this.afficher_snackbar = true;
                        this.getListeDesCollecteursQuartier();
            ...
            axios.post('/valider_desaffection_collecteur_quartier', {
                ...
                .then(response => {
                    ...
                    if (response.data.success) {
                        ...
                        this.message_snackbar = "La désaffectation a été effectué."
                        this.afficher_snackbar = true;
            ...
            axios.post('/valider_affection_collecteur_quartier', {
                ...
                .then(response => {
                    ...
                    if (response.data.success) {
                        ...
                        this.message_snackbar = "L'affectation a été effectué."
                        this.afficher_snackbar = true;
            ...
            if (this.value_quartier.length <= 0) {
                ...
                if(this.aff_reaf_des > 1)
                    ...
                    this.data_affectation.forEach((item_aff, index) => {
                        ...
                        if(this.verifierEstDejaAttribueCollecteur(item_aff)){
                            this.data_affectation.splice(this.data_affectation.indexOf(item_aff), 1);
                            this.data_collecteur.push(item_aff);
                        }
        ...
        getListeDesCollecteursQuartier() {
            ...
            axios.post('/get_collecteur_du_quartier', {id_qt: this.value_quartier.map(a => a.id_qt)}).then(response => {
                ...
                response.data.data.forEach((item, index) => {
                    ...
                    this.data_collecteur.forEach((item_user, index) => {
        ...
        getInfoUser() {
            ...
            axios.post('/get_info_coll', {data: JSON.stringify(this.user), page: this.page}).then(response => {
        ...
        verifierEstDejaAttribueCollecteur(item){

            let nv_qt = this.value_quartier.map(a => a.id_qt);
            let a_deja_quartier = 0;

            item.id_qt.forEach((anc_qt, index1) => {


                nv_qt.forEach((nv_qt, index2) => {
        ...
        erreurAlert(){
            ...
            let message = "";
            if (this.value_quartier.length > 1)
                message = "Ce collecteur est déjà attribué aux quartiers sélectionnés";
            else
                message = "Ce collecteur est déjà attribué à ce quartier sélectionné";
