    ...
    <v-card fluid :disabled="load_send || loading_data || load_data_zone">
        ...
        <v-row>
                ...
                <v-card>
                    ...
                    >
                        ...
                        <template v-slot:no-data>
                            <p class="subtitle-1">Aucun collecteur dans la base de données</p>
                        </template>
            ...
            <v-col cols="6">
                ...
                <div   v-if="aff_reaf_des === 1 && data_affectation.length <= 0">
                    ...
                    <p class="text-h6 mt-6">Liste vide! Choisissez les collecteurs à affecter au(x) zone(s) sélectionnée(s)</p>
                ...
                >
                    ...
                    <template v-if="aff_reaf_des !== 1" v-slot:top>
                        <v-toolbar
                            flat
                        >
                            <v-toolbar-title class="font-weight-black">Liste des collecteurs {{ value_zone.length > 1  ? ' affectés aux  zones' : 'affecté à la zone' }} sélectionnée{{ value_zone.length > 1  ? 's' : '' }}. </v-toolbar-title>
                ...
                <div v-show="value_zone.length > 0 && aff_reaf_des > 1 && data_affectation.length <= 0">
                    <p  class="headline font-weight-black">Aucun  collecteur est affecté à {{value_zone.length > 1 ? "ces ("+ value_zone.length+"  zones )" : "cette zone"}}</p>
        ...
        <v-row justify="center">
            ...
            >
                <v-card>
                    <v-card-title class="headline">
                        {{data_r_affectation.length> 1 ? 'Attention les collecteurs ne seront plus rattachés à une zone ' :
...
export default {
    ...
    methods: {
        ...
        getListeDesCollecteursZone() {
            ...
            axios.post('/get_collecteur_zone', {id_zn: this.value_zone.map(a => a.id_zn)}).then(response => {
        ...
        getInfoCollNonAss() {
            ...
            this.loading_data = true;
            axios.post('/get_coll_non_ass_zone').then(response => {
        ...
        erreurAlert(){
            ...
            if (this.value_zone.length > 1)
                message = "Ce collecteur est déjà attribué aux zones sélectionnées";
            else
                message = "Ce collecteur est déjà attribué à la zone sélectionnée";
